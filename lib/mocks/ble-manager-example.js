import { MockBleManager } from './mock-ble-manager.js';
...
const stateSubscription = bleManager.onStateChange(state => console.log(`State changed: ${state}`), true);
...
setTimeout(() => {
    ...
    bleManager.startDeviceScan(['180D'], // Only look for heart rate services
...
setTimeout(async () => {
    ...
    bleManager.setCharacteristicValueForReading(deviceId, serviceUUID, characteristicUUID, Buffer.from([0x06, 0x48]).toString('base64') // [Flags, Heart Rate]
    );
    // Monitor the characteristic
    console.log('Monitoring heart rate characteristic...');
    const subscription = bleManager.monitorCharacteristicForDevice(deviceId, serviceUUID, characteristicUUID, (error, characteristic) => {
        if (error) {
    ...
    setTimeout(() => {
        ...
        bleManager.setCharacteristicValue(deviceId, serviceUUID, characteristicUUID, Buffer.from([0x06, 0x52]).toString('base64') // 82 bpm
        );
    }, 4000);
    // Simulate an error after 6 seconds
    setTimeout(() => {
        console.log('Simulating characteristic error');
        bleManager.simulateCharacteristicError(deviceId, serviceUUID, characteristicUUID, new Error('Connection lost'));
    }, 6000);
...
setTimeout(async () => {
    ...
    bleManager.setCharacteristicValueForReading(deviceId, serviceUUID, characteristicUUID, Buffer.from([0x06, 0x4B]).toString('base64') // 75 bpm
    );
    ...
    // Simulate read error
    bleManager.simulateCharacteristicReadError(deviceId, serviceUUID, characteristicUUID, new Error('Read failed: Insufficient permissions'));
...
setTimeout(async () => {
    ...
    const writeListener = bleManager.onCharacteristicWrite(deviceId, serviceUUID, characteristicUUID, value => {
    ...
    try {
        await bleManager.writeCharacteristicWithResponseForDevice(deviceId, serviceUUID, characteristicUUID, Buffer.from([0x01, 0x02]).toString('base64'));
    ...
    try {
        await bleManager.writeCharacteristicWithoutResponseForDevice(deviceId, serviceUUID, characteristicUUID, Buffer.from([0x03, 0x04]).toString('base64'));
    ...
    bleManager.simulateWriteWithResponseError(deviceId, serviceUUID, characteristicUUID, new Error('Write failed: Device not connected'));
    console.log('Writing with response (simulated error)...');
    try {
        await bleManager.writeCharacteristicWithResponseForDevice(deviceId, serviceUUID, characteristicUUID, Buffer.from([0x05, 0x06]).toString('base64'));
    }
    catch (error) {
...
setTimeout(async () => {
    ...
    const disconnectionSubscription = bleManager.onDeviceDisconnected(deviceId, (error, device) => {
        if (error) {
    ...
    try {
        ...
        console.log('Reading characteristic...');
        try {
            const char = await bleManager.readCharacteristicForDevice(deviceId, '180D', '2A37');
    ...
    setTimeout(async () => {
        bleManager.simulateConnectionError(deviceId, new Error('Connection failed: Device unreachable'));
...
setTimeout(async () => {
    ...
    const mtuSubscription = bleManager.onMTUChanged(deviceId, mtu => console.log(`MTU changed to: ${mtu}`));
...
setTimeout(() => {
    ...
    const restoredManager = new MockBleManager({
        ...
        restoreStateFunction: (state) => {
            if (state) {
                console.log('State restored with connected devices:', state.connectedPeripherals.map(d => d.name));
            }
            else {
    ...
    setTimeout(() => {
        ...
        const newManager = new MockBleManager({
            ...
            restoreStateFunction: (state) => {
                if (state) {
                    console.log('After restart - restored devices:', state.connectedPeripherals.map(d => d.name));
                }
                else {
...
//# sourceMappingURL=ble-manager-example.js.map