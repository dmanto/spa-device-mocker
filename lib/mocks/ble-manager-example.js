...
bleManager.startDeviceScan(['180D'], { allowDuplicates: true }, (error, device) => {
    if (error)
...
bleManager.setCharacteristicValue('heart-monitor-123', '180D', '2A37', Buffer.from([0x06, 0x48]).toString('base64'));
const subscription = bleManager.monitorCharacteristicForDevice('heart-monitor-123', '180D', '2A37', (error, char) => {
    if (error)
...
setTimeout(() => {
    ...
    subscription.remove();
    bleManager.stopSimulatedNotifications('heart-monitor-123', '180D', '2A37');
}, 5000);
...
setTimeout(async () => {
    ...
    try {
        const result = await bleManager.writeCharacteristicWithResponseForDevice(deviceId, serviceUUID, characteristicUUID, Buffer.from([0x01, 0x02]).toString('base64'));
    ...
    try {
        const result = await bleManager.writeCharacteristicWithoutResponseForDevice(deviceId, serviceUUID, characteristicUUID, Buffer.from([0x03, 0x04]).toString('base64'));
