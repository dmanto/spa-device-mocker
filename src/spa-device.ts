// models/spa-devices.ts
import { DeviceState, DeviceMode, Characteristic, MModeCommand } from '../types';
...
export class SpaDevices {
  ...
  private devices: Map<string, SpaDevices> = new Map();
  

  constructor(mac: string, area: string = 'default', rssi: number = -70) {
    this.mac = mac;
    this.area = area;
    this.rssi = rssi;
  ...
  private handlePartialWrite(characteristic: Characteristic, value: string): boolean {
  ...
  // ... other methods (handleMcodeWrite, handleMmodeCommand, etc.)
}